...
複雜的 Notion 資料結構解析 (Recursion)：

捨棄低效的全域掃描，精準抓取前 100 個區塊。

運用遞迴 (Recursive) 函數，成功解析 Notion 中常見的巢狀結構（如 Toggle 摺疊列表）與多層次標題。
...
雙版本對照 (Ollama vs. Gemini)：
同時提供地端模型 (Ollama) 與雲端 API (Gemini) 雙版本，展現對不同 LLM 部署架構的理解與實作能力。

🚀 系統架構與流程
觸發：執行 Python 腳本。
提取 (Notion API)：透過 API 讀取特定 Database ID 的內容，尋找符合今日日期的日記區塊。
清洗：過濾空白內容，將零散的 Rich Text 碎片黏合成純文字。
分析 (Gemini API / Ollama)：將純文字送入 LLM 進行情緒鑑定。
輸出：回傳 0~10 分的情緒分數與具體評語，並透過系統底層編碼設定 (sys.stdout) 確保終端機完美輸出 UTF-8 與 Emoji。
